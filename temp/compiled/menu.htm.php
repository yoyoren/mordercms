...
<?php echo $this->smarty_insert_scripts(array('files'=>'utils.js,global.js')); ?>
<style type="text/css">
body {
  background: #80BDCB;
...
.explode {
  background: url(images/menu_minus.gif) no-repeat 0px 10px;
  font-weight: bold;
}
.collapse {
  background: url(images/menu_plus.gif) no-repeat 0px 10px;
  font-weight: bold;
}
.menu-item {
  background: url(images/menu_arrow.gif) no-repeat 0px 10px;
  font-weight: normal;
}
#help-title {
  font-size: 14px;
...
<body>
<div id="tabbar-div">
<p><span style="float:right; padding: 3px 5px;" ><a href="javascript:toggleCollapse();"><img id="toggleImg" src="images/menu_minus.gif" width="9" height="9" border="0" alt="收起" /></a></span>
...
<?php if ($this->_var['menu']['action']): ?>
  <li class="explode"><a href="<?php echo $this->_var['menu']['action']; ?>" target="main"><?php echo $this->_var['menu']['label']; ?></a></li>
<?php else: ?>
  <li class="explode" key="<?php echo $this->_var['k']; ?>" name="menu">
    <?php echo $this->_var['menu']['label']; ?>
    <?php if ($this->_var['menu']['children']): ?>
    <ul class="menu-chilchren">
...
?>
      <li class="menu-item"><a href="<?php echo $this->_var['child']['action']; ?>" target="main"><?php echo $this->_var['child']['label']; ?></a></li>
...
ToggleHanlder.CookieName += "_<?php echo $this->_var['mytp_admin_id']; ?>";
//初始化菜单状态
ToggleHanlder.Load();
