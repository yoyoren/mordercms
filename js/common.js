...
function checkOrder()
{
  var lastCheckOrder = new Date(document.getCookie('Los_LastCheckOrder'));
  var today = new Date();

  if (lastCheckOrder == null || today-lastCheckOrder >= NEW_ORDER_INTERVAL)
  {
    document.setCookie('Los_LastCheckOrder', today.toGMTString());

    try
    {
      Ajax.call('sync.php?is_ajax=1&act=auto','', checkOrderResponse, 'GET', 'JSON');
    }
    catch (e) { }
  }
}
