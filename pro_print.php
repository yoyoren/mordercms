    ...
    {	
		...
		{   
			...
			for($i=1;$i<=$cakesum;$i++){
			  ...
			  //$cakesum_sn=$cakesum_array[$i];
			  //$smarty->assign('cake_sn',    $cakesum_sn);
			}
			 //echo $cakesum;
			// print_r($cakesum_array); 
		   
			$sql = "select a.turn,b.route_name,a.add_time from order_dispatch as a,ship_route as b where a.route_id=b.route_id and a.order_id=".$order_id;
			$pack = $db_read->getRow($sql);	
			$pack['add_time']=date('Y-m-d H:i',$pack['add_time']);
			foreach ($res as $key=>$val)
			{
			  ...
			  // $smarty->assign('candle',    $candle);
			   $smarty->assign('cakesum',    $cakesum);		  
			   $smarty->assign('cake_sn',    $cakesum_array[$key]);
