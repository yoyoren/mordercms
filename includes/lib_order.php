...
function shipping_list()
{
    ...
    return $GLOBALS['db_read']->getAll($sql);
}
...
function shipping_info($shipping_id)
{
    ...
    return $GLOBALS['db_read']->getRow($sql);
}
...
function shipping_area_info($shipping_id, $region_id_list)
{
    $sql = 'SELECT s.shipping_code, s.shipping_name, ' .
            ...
            ' AND r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id AND s.enabled = 1';
    $row = $GLOBALS['db_read']->getRow($sql);
...
function available_payment_list($support_cod, $cod_fee = 0, $is_online = false)
{
    ...
    $res = $GLOBALS['db_read']->query($sql);

    $pay_list = array();
    while ($row = $GLOBALS['db_read']->fetchRow($res))
    {
        if ($row['is_cod'] == '1')
        {
            $row['pay_fee'] = $cod_fee;
...
function order_amount($order_id, $include_gift = true)
{
    ...
    return floatval($GLOBALS['db_read']->getOne($sql));
}
...
function order_fee($order, $goods, $consignee)
{
    ...
    if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
        ...
        if (!empty($shipping_info))
        {
            ...
            $shipping_count = $GLOBALS['db_read']->getOne($sql);
    ...
    if ($order['extension_code'] == 'exchange_goods')
    {
        ...
        $exchange_integral = $GLOBALS['db_read']->getOne($sql);
...
{
    return $GLOBALS['db_write']->autoExecute($GLOBALS['ecs']->table('order_info'),
...
function cart_goods($type = CART_GENERAL_GOODS)
{
    ...
    $arr = $GLOBALS['db_read']->getAll($sql);
...
function cart_goods_exists($id, $spec, $type = CART_GENERAL_GOODS)
{
    ...
    return ($GLOBALS['db_read']->getOne($sql) > 0);
}
...
function cart_weight_price($type = CART_GENERAL_GOODS)
{
    ...
    $row = $GLOBALS['db_read']->getAll($sql);
    ...
    if ($row)
    {
        ...
        foreach ($row as $val)
        {
            ...
            if ($shipping_count > 0)
            {
                ...
                $goods_row = $GLOBALS['db_read']->getRow($sql);
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    $sql = "SELECT g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, ".
            ...
            " AND g.is_delete = 0";
    $goods = $GLOBALS['db_read']->getRow($sql);
    ...
    if ($parent > 0)
    {
        ...
        if ($GLOBALS['db_read']->getOne($sql) == 0)
        {
    ...
    $prod = $GLOBALS['db_read']->getRow($sql);
    ...
    foreach ($basic_list as $parent_id => $fitting_price)
    {
        ...
        /* 添加 */
        $GLOBALS['db_write']->autoExecute($GLOBALS['ecs']->table('cart'), $parent, 'INSERT');
