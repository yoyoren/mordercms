...
function order_list()
{
   if (!empty($_GET['is_ajax']) && $_GET['is_ajax'] == 1)
   {
	...
	$sql = "select o.order_amount,o.pay_note,o.add_time as addt,o.money_paid,o.pay_id ".
	       "from order_delivery as a ".
		   "left join ecs_order_info as o on a.order_id=o.order_id ".
	       "left join order_dispatch as c on a.order_id=c.order_id ".
		   "left join print_log_x as p on a.order_id=p.order_id ".
	...
	foreach($pos as $vl)
	{
		if(strpos($vl['pay_note'],'OS'))
		{
		   $ofee += floatval(substr($vl['pay_note'],7,8));
			...
			if(strpos($vl['pay_note'],'现')!==false)
			{
				$sums +=floatval(substr($vl['pay_note'],7,8));
			}else{
				if(in_array($vl['pay_id'],array(10,25,28,8,30))){
					$ofee +=$vl['money_paid'];
				}
				$sums += $vl['order_amount'];
			}
		}
	...
	$sql = "select *,o.order_id,o.add_time as addt,o.money_paid,o.pay_id,a.status as status,p.print_sn ".
	       "from order_delivery as a ".
		   "left join ecs_order_info as o on a.order_id=o.order_id ".
	       "left join order_dispatch as c on a.order_id=c.order_id ".
		   "left join print_log_x as p on a.order_id=p.order_id ".
    ...
    foreach($res as $key => $val)
	{
		...
		if(in_array($val['pay_id'],array(10,25,28,8,30))){
			
			$res[$key]['order_amount'] = $val['money_paid']+$val['order_amount'];
