...
/*$pay=pay_info($id);
print_r($pay);
$smarty->assign('payname',		$pay['payname']);*/
$order = $db_read->getRow($sql);
$sql="select count(*) from tender_info where order_id='$id'";

$num= $db_read->getOne($sql);
if($num==1){
  $sql="select pay_name from tender_info where order_id='$id'";
  $pay_name=$db_read->getOne($sql);  
}else if($num>1){
  $sql="select pay_name from tender_info where order_id='$id'";
  $pay_name=$db_read->getAll($sql);
  foreach($pay_name as $k=>$v){
    //$paystr=",".$v['pay_name'].",";
	if($k!=count($rs)-1){
     $paystr.=$v['pay_name'].",";
   }else{
...
while ($row = $db_read->fetchRow($res))
{
   ...
   else
   {
			 ...
			 $row['formated_subtotal_new']       = $row['goods_price'] * $row['goods_number']*$row['goods_discount'];
			 }
			 else
			 {
			 $row['formated_subtotal_new']       = floor($row['goods_price'] * $row['goods_number']-$row['goods_discount']);
...
$smarty->display('order_info1.html');
